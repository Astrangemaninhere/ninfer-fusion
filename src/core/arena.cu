...
DeviceSpan DeviceArena::alloc_bytes(std::size_t bytes, std::size_t align) {
    ...
    if (bytes == 0) {
        ...
        Dl_info info{};
        std::size_t offset = 0;
        const bool have_base = dladdr(caller, &info) != 0 && info.dli_fbase != nullptr;
        if (have_base) {
            offset = static_cast<std::size_t>(static_cast<const char*>(caller) -
                                              static_cast<const char*>(info.dli_fbase));
        }
        if (have_base && info.dli_sname != nullptr) {
            std::fprintf(stderr, "arena alloc-zero caller %s+%p module=0x%zx\n", info.dli_sname,
                         static_cast<char*>(caller) - static_cast<char*>(info.dli_saddr), offset);
        } else {
            std::fprintf(stderr, "arena alloc-zero caller %p module=0x%zx\n", caller, offset);
        }
        void* frames[24];
