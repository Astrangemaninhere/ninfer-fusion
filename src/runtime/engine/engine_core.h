...
public:
    ...
    // S31: also surfaces the 3.1 category and the last request-domain failure.
    // `reason` must stay off auth-exempt surfaces: /health renders `category`
    // only (see http_server.cpp); `reason` is for logs and authenticated API
    // callers.
    [[nodiscard]] EngineFailureState failure_state() const {
        std::lock_guard lock(queue_mutex_);
        EngineFailureState state;
        state.last_request_error = last_request_failure_;
        if (failure_reason_.empty()) { return state; }
        state.failed   = true;
        state.reason   = failure_reason_;
        state.since    = failed_at_;
        state.category = failure_category_;
        return state;
    }
...
private:
    ...
    [[nodiscard]] static FailureClass classify_failure(const std::exception_ptr& error) noexcept {
        ...
        try {
            ...
            // 3.1 resource domain: exhausted allocation retries surface as
            // runtime_error from the arena/DeviceBuffer retry loops
            // (src/core/arena.cu: "cudaMalloc failed"/"cudaMallocHost failed",
            // 6 attempts x 2s). Prefix match is deliberate: those strings are
            // constructed only at allocation sites, and a missed match merely
            // downgrades the label to "invariant" - the engine stops either way.
            const std::string_view message = error.what();
            if (message.starts_with("cudaMalloc failed") ||
                message.starts_with("cudaMallocHost failed")) {
                return FailureClass::Resource;
            }
    ...
    [[nodiscard]] static const char* failure_class_label(FailureClass failure_class) noexcept {
        switch (failure_class) {
        case FailureClass::Request: return "request";
        case FailureClass::Resource: return "resource";
        case FailureClass::Invariant: return "invariant";
        }
        return "unknown";
    }

    [[nodiscard]] static const char* host_work_class_name(HostWorkClass work_class) noexcept {
        switch (work_class) {
        case HostWorkClass::Decode: return "decode";
        case HostWorkClass::Prefill: return "prefill";
        case HostWorkClass::Control: return "control";
        }
        return "boundary";
    }
    ...
    AdmissionProgress try_admit_one() {
        ...
        for (;;) {
            ...
            try {
                // S35 (3.4): the admission-planning phase is uniformly request-scoped —
                // candidate inspection joins base planning inside the per-request try,
                // so a planning exception fails only this request instead of reaching
                // the worker boundary with no owner (engine stall).
            ...
            for (const std::shared_ptr<Request>& candidate : queued.backfill_candidates()) {
                ...
                try {
                    // S35 (3.4): same request scoping as the FIFO-head path above.
    ...
    bool fail_request_lane(const std::exception_ptr& error) {
        ...
        // 3.5: desensitized last_request_error telemetry (id/phase/time only)
        // so the auth-exempt /health can prove the engine survived a lane
        // failure without exposing exception text.
        {
            std::lock_guard lock(queue_mutex_);
            last_request_failure_.occurred   = true;
            last_request_failure_.request_id = request->id;
            last_request_failure_.phase      = host_work_class_name(current_host_work_class_);
            last_request_failure_.at         = std::chrono::system_clock::now();
        }
        std::fprintf(stderr,
                     "[engine] request-domain failure class=request phase=%s id=%llu lane=%u: "
                     "%s\n",
                     host_work_class_name(current_host_work_class_),
    ...
    void fail_all_locked(std::exception_ptr error) noexcept {
        ...
        // 3.1/3.5: classify at the worker boundary and log type + phase +
        // reason server-side (once /health is desensitized this fprintf is the
        // only place an operator reads the raw reason). Categories label
        // engine-stop failures only; the shutdown sentinel (RequestErrorKind::
        // Unavailable from the stopping_ path) is the sole request-class
        // exception arriving here and stays uncategorized.
        const FailureClass failure_class = classify_failure(error);
        std::fprintf(stderr, "[engine] engine failure class=%s phase=%s reason: %s\n",
                     failure_class_label(failure_class),
                     host_work_class_name(current_host_work_class_), reason.c_str());
        {
            std::lock_guard lock(queue_mutex_);
            failed_ = true;
            if (failure_reason_.empty()) {
                failure_reason_   = std::move(reason);
                failure_category_ = failure_class == FailureClass::Request
                                        ? std::string()
                                        : failure_class_label(failure_class);
    ...
    std::string failure_category_;                      // S31 3.1 label (guard: queue_mutex_)
    EngineRequestFailureNote last_request_failure_{};   // S31 3.5 (guard: queue_mutex_)
