...
#define NINFER_QWEN36_VARIANT    ::ninfer::targets::muse_glimmer_30b::detail::Variant
#define NINFER_QWEN36_RUNTIME_NS muse_glimmer_30b_runtime
...
std::array<DType, 64> Variant::default_layer_kv_dtypes(WeightsProfile) {
    // Muse declares supports_per_layer_kv_defaults=false; the planner never
    // calls this. A uniform NVFP4 default is provided for layout planning.
...
void Variant::attention_projection(const Tensor& hidden,
    ...
    // Muse: 4 独立投影 (引擎融合内核为 qwen 固定几何; 通用 linear 无约束).
...
void Variant::attention_output_projection(const Tensor& attention, const Weight& weight,
                                          ...
                                          WorkspaceArena& workspace, cudaStream_t stream) {
    // linear_add 的 FP8 profile 为 qwen 固定几何 -> Muse 拆通用 linear + 残差.
    (void)workspace;
...
void Variant::post_mixer(const Tensor& hidden, const PostMixerWeights& weights, Tensor& residual,
    ...
    auto scope        = workspace.scope();
    const int cols    = hidden.ne[1];
    // linear_swiglu 无 FP8 workspace 路线 -> 拆独立 linear + silu_mul (通用格式).
    ...
    ops::silu_mul(gate, up, stream);
    Tensor o = workspace.alloc(DType::BF16, {TextConfig::hidden, cols});
    ops::linear(up, weights.down, o, stream);
    ops::residual_add(o, residual, stream);
}
...
std::size_t Variant::attention_projection_workspace_capacity_bytes(WeightsProfile,
    ...
    // Muse 投影 = 4 次独立通用 linear (无 workspace).
    return 0;
}
...
std::size_t Variant::attention_output_projection_workspace_capacity_bytes(
    ...
    validate_token_interval(first, last);
    // Muse: 通用 linear + residual (无 workspace profile 约束).
    return 0;
}
...
std::size_t Variant::post_mixer_workspace_capacity_bytes(WeightsProfile weights_profile,
    ...
    case WeightsProfile::MuseNvfp4:
        return post_mixer_workspace_bytes(QType::FP8_E4M3FN_ROW_BF16S,
                                          QType::FP8_E4M3FN_ROW_BF16S, kFp8TextPolicy, first,
                                          last);
    }
    throw std::invalid_argument("muse_glimmer_30b: invalid weights profile");
}
