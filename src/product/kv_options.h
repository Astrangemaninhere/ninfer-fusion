...
// where <type> is bf16, int8, fp8, nvfp4, iso3 or e8. Unlisted slots stay
...
//
// Caveat (known): BFloat16 doubles as the "unset" sentinel, so an explicit
// `all:bf16` is indistinguishable from an empty spec and the layers keep
// inheriting the global dtype. To force a BF16 baseline pass the global flag
// `--kv-dtype bf16` (which now reaches the pool geometry, _TODO.md 98/117 U3);
// giving BF16 its own per-layer spelling needs a separate "was set" mask in
// this API.
