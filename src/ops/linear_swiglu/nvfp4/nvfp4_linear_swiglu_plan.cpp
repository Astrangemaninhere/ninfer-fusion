...
Nvfp4LinearSwiGluWorkspace allocate_baseline_workspace(Allocator& allocator, std::int32_t tokens) {
    ...
    // A 1..4 token tail takes the A16 route and needs no W4A4 workspace, but the
    // borrowed arena must stay non-empty; keep a minimal floor. The planner only
    // sizes this branch for >= 49 tokens, where linear_bytes dominates.
    out.linear = allocator.alloc_bytes(std::max<std::size_t>(linear_bytes, 256), 256);
    return out;
}
...
std::size_t nvfp4_linear_swiglu_workspace_capacity_bytes(LinearPolicy policy,
    ...
    // UNIFY-A: T<=4 now takes the fused W4A4 route (which needs the activation
    // quantize workspace) instead of the A16 route, so the floor is gone.
