...
ServeOptions parse_serve_options(int argc, char** argv) {
    ...
    for (int i = 2; i < argc; ++i) {
        ...
        if (arg == "--host") {
            ...
            // value (Window/Disk: cold_keep_tokens/page_size + 16). ColdPolicy
            // ::None ignores this (layouts_impl forces 0 -- no cold path at all).
            ...
            // W13 P0: parsed for forward-compat, but offload CANNOT work yet
            // (the per-GEMM residency hook is P1, _collab/A_s32_w13_p0.md).
            // Fail loudly instead of silently serving fully resident -- a
            // silent no-op offload flag is exactly the failure mode W13 forbids.
            const std::uint64_t bytes =
                parse_u64(require_value("--weight-host-bytes"), "weight-host-bytes");
            if (bytes != 0) {
                throw std::invalid_argument(
                    "--weight-host-bytes requires the W13 GEMM residency hook (P1); "
                    "refusing to run with weights silently fully resident");
            }
        } else if (arg == "--yarn") {
