// qpn_probe5.cu — lane 0-15 每 ci 的完整 (col,k) 表 (G=4, K=64).
...
int main() {
    ...
    for (int lane = 0; lane < 16; lane += 5) {   // 抽样 lane 0,5,10,15
        for (int ci : {0, 1, 4, 8, 12}) {
            for (int g = 0; g < G; ++g) {
                std::vector<unsigned char> codes(G * 32 * 16, 0), scales(G * 32, 0x38);
                codes[(g * 32 + lane) * 16 + ci] = 0x07;
                ...
                for (int kp = 0; kp < 16; ++kp) {
                    ...
                    for (int j = 0; j < N; ++j) {
                        if (std::fabs(__half2float(y[j])) > 1e-3f) {
                            std::printf("lane %2d g%d ci%2d -> col %2d k %2d (w=%.1f)\n", lane,
                                        g, ci, j, kp, __half2float(y[j]));
                        }
                    }
                }
            }
