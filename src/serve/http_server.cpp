...
void HttpServer::register_routes() {
    ...
    server_.Get("/health", [this](const httplib::Request&, httplib::Response& res) {
        // W16: a poisoned engine is not healthy — report state so orchestration
        // can drain the instance instead of retrying a dead one.
        // S31 3.5: /health is auth-exempt (pre-routing handler above), so the
        // payload carries the 3.1 category and NEVER the raw reason — no paths,
        ...
        if (service_ != nullptr) {
            ...
            if (failure.failed) {
                engine["category"] = failure.category;
                engine["since"]    = std::chrono::duration_cast<std::chrono::seconds>(
                                        failure.since.time_since_epoch())
                                        .count();
            }
            if (failure.last_request_error.occurred) {
                engine["last_request_error"] = {
                    {"request_id", failure.last_request_error.request_id},
                    {"phase", failure.last_request_error.phase},
                    {"since", std::chrono::duration_cast<std::chrono::seconds>(
                                  failure.last_request_error.at.time_since_epoch())
                                  .count()},
                };
            }
            if (failure.failed) { res.status = 503; }
            res.set_content(
                nlohmann::json{{"status", failure.failed ? "failed" : "ok"}, {"engine", engine}}
                    .dump(),
                "application/json");
            return;
        }
