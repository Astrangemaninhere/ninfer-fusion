p = '/home/user/ninfer-fusion/src/targets/qwen3_6/impl/runtime/decode_impl.h'
s = open(p).read()

# body gains segment parameters; sample/scatter/egress only on final segment
old_body = '''auto ordinary_batch_body(OrdinaryBatchContext& state, std::int32_t batch_size,
...
new_body = '''auto ordinary_batch_body(OrdinaryBatchContext& state, std::int32_t batch_size,
...
# segmented capture
old_cap = '''void capture_ordinary_decode_batch(OrdinaryBatchContext& state, std::int32_t batch_size,
                                   ops::GqaExecutionEnvelope envelope,
                                   DecodeGraphDefinition& definition) {
    auto body = ordinary_batch_body(state, batch_size, envelope);
    capture_graph(state, definition, body);
}'''
new_cap = '''void capture_ordinary_decode_batch(OrdinaryBatchContext& state, std::int32_t batch_size,
                                   ops::GqaExecutionEnvelope envelope,
                                   DecodeGraphDefinition& definition) {
...
open(p, 'w').write(s)
print('decode_impl segmented capture')
