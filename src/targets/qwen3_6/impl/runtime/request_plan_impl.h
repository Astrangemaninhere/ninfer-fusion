...
// S35 (3.4): request-domain planning validations raise RequestError so the
// exception stays in the request domain at the worker boundary; messages are
// byte-identical to the previous invalid_argument texts.
...
void validate_sampling(const ResolvedSamplingParameters& sampling) {
    if (!std::isfinite(sampling.temperature) || !std::isfinite(sampling.top_p) ||
        ...
        throw RequestError(RequestErrorKind::InvalidPrompt,
                           "sampling parameters must be finite");
    }
    if (sampling.top_p < 0.0F || sampling.top_p > 1.0F) {
        throw RequestError(RequestErrorKind::InvalidPrompt, "top_p must be in [0,1]");
    }
    if (sampling.min_p < 0.0F || sampling.min_p > 1.0F) {
        throw RequestError(RequestErrorKind::InvalidPrompt, "min_p must be in [0,1]");
    }
}
...
RequestBasePlan ProgramImplCore::plan_request(const PreparedPromptData& prompt,
    ...
    if (prompt.token_ids.empty()) {
        throw RequestError(RequestErrorKind::InvalidPrompt, "prompt must contain tokens");
    }
    ...
    for (const TokenId id : prompt.token_ids) {
        if (id < 0 || id >= TextConfig::token_domain) {
            throw RequestError(RequestErrorKind::InvalidPrompt,
                               "prompt contains token outside the registered token domain");
        }
    }
    ...
    if (prompt.has_media() && !vision_enabled) {
        throw RequestError(RequestErrorKind::InvalidMedia,
                           "Vision is disabled for this Engine");
    }
    ...
    if (options.allow_prefix_reuse && prompt.identity.reusable && context_cache.enabled) {
        const auto add_capture = [&](std::uint32_t frontier, std::uint32_t input_order,
            ...
            if (frontier == 0 || frontier > base->summary.prompt_tokens) {
                throw RequestError(RequestErrorKind::InvalidPrompt,
                                   "capture opportunity frontier is invalid");
            }
