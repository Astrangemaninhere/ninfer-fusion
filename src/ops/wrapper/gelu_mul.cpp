// ninfer::ops — gelu_mul wrapper: implements the public api, validates parameters, and
...
void gelu_mul(const Tensor& gate, const Tensor& up, Tensor& out, cudaStream_t stream) {
    ...
    if (!out.is_contiguous()) { throw std::invalid_argument("gelu_mul: out must be contiguous"); }
    ...
    detail::gelu_and_mul_launch(gate, up, out, stream); // single variant -> direct dispatch
}
