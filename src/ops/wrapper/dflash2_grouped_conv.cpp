...
void dflash2_grouped_conv(const Tensor& hidden, const Tensor& delta, const Weight& base,
    ...
    // block_size is the live request width W = K + 1 and must track the
    // startup-selected K, not the checkpoint's recommended 8 (the masked-block
    // algorithm only requires the conv boundary to match the request boundary).
    // The kernel derives the in-block position as `t & (block_size - 1)`, so W has
    // to stay a power of two: this admits K in {1,3,7,15}.
    if (hidden_size != 5120 || tokens < 1 || tokens > 64 || block_size < 2 ||
        block_size > 16 || group_size != 16 || taps != 2 || side < 0 || side > 1 ||
        (block_size & (block_size - 1)) != 0) {
        throw std::invalid_argument(
            "dflash2_grouped_conv: registered domain is H=5120, T=1..64, taps=2, G=16, "
            "block=2|4|8|16, side in {0,1}");
    }
