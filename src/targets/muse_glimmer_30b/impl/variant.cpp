...
std::array<DType, 64> Variant::default_layer_kv_dtypes(WeightsProfile) {
    ...
    // calls this. A uniform BF16 default (matches serve --kv-dtype bf16);
    // per-layer KV quantization tables can be introduced separately.
...
void Variant::mtp_post_mixer(const Tensor&, const MtpPostMixerWeights&, Tensor&,
    ...
    throw std::logic_error("MTP is not enabled for muse_glimmer_30b");
}
