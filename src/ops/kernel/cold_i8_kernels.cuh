...
// `Groups` is the geometry's 64-dim group count (4 at head_dim 256, 2 at 128).
// The slot row itself is a fixed 256-wide layout (128 B of nibbles + 16 B of E4M3
// sub-scales), so the per-group inner loop is unchanged; only the outer group count
// follows the caller's arrays (S36 made the caller's row_scales geometry-sized).
