...
void ProgramImplCore::enqueue_dflash_context_append(std::span<const std::uint32_t> lanes,
    ...
    *dflash_host_ingress        = {};
    for (std::size_t row = 0; row < lanes.size(); ++row) {
        ...
        ensure_sequence_kv_mapped(sequence, std::max(sequence.text_kv_valid, end), end);
...
ProgramImplCore::decode_dflash2_batch(std::span<const std::uint32_t> lanes,
    ...
    try {
        ...
        schedule::dflash2_decode_batch(schedule_state, static_cast<std::int32_t>(lanes.size()),
                                       draft_window, envelopes, target_envelope, executable);
        // [T3 probe · env-gated] NINFER_DF2DBG=1 prints the DFlash2 round's draft/verify
        // position family (column, position, verify id, draft, target argmax) so the
        // "one position off" question is settled by one run instead of by inference.
        // Zero behaviour change: the copies below are synchronous, env-gated, and run
        // outside the CUDA-graph capture (after the batch call, before the wait).
        if (std::getenv("NINFER_DF2DBG") != nullptr) {
            const auto probe_i32 = [](const Tensor& tensor, std::int32_t index) {
                std::int32_t value = 0;
                CUDA_CHECK(cudaMemcpy(&value,
                                      static_cast<const std::int32_t*>(tensor.data) + index,
                                      sizeof(std::int32_t), cudaMemcpyDeviceToHost));
                return value;
            };
            const std::int32_t probe_width = static_cast<std::int32_t>(width);
            for (std::int32_t row = 0; row < static_cast<std::int32_t>(lanes.size()); ++row) {
                std::fprintf(stderr,
                             "[df2dbg] row=%d anchor=%d frontier=%d valid=%d in_extent=%d "
                             "out_extent=%d accepted=%d count=%d\n",
                             row, probe_i32(schedule_state.frame.anchors, row),
                             probe_i32(schedule_state.frame.execution_frontiers, row),
                             probe_i32(schedule_state.frame.target_valid_columns, row),
                             dflash2_host_ingress->proposal_extents[row],
                             dflash2_host_egress->proposal_extents[row],
                             dflash2_host_egress->accepted_drafts[row],
                             dflash2_host_egress->licensed_counts[row]);
                for (std::int32_t column = 0; column < probe_width; ++column) {
                    const std::int32_t offset = row * probe_width + column;
                    const std::int32_t draft =
                        column + 1 < probe_width
                            ? probe_i32(schedule_state.frame.draft_tokens,
                                        row * (probe_width - 1) + column)
                            : -1;
                    std::fprintf(stderr, "[df2dbg]   col=%d pos=%d verify=%d draft=%d argmax=%d\n",
                                 column,
                                 probe_i32(schedule_state.frame.proposal_positions, offset),
                                 probe_i32(schedule_state.frame.verify_ids, offset), draft,
                                 probe_i32(schedule_state.frame.target_argmax, offset));
                }
            }
        }
        submit_range.reset();
