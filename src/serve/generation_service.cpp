...
ApiError request_error_to_api_error(const ninfer::RequestError& exception) {
    ...
    case ninfer::RequestErrorKind::ContextLengthExceeded:
        ...
        break;
    case ninfer::RequestErrorKind::InvalidPrompt:
        // Same wire identity the prepare path already uses for planner-rejected
        // prompts (throw_invalid_input "invalid_prompt"), so log greps match.
        error.status = 400;
