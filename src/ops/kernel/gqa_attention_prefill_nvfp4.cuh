...
__launch_bounds__(256) __global__
    ...
    const int units             = tokens * Geometry::KVHeads * (Geometry::HeadDim / kGqaKvNvfp4Group);
    if (unit >= units) { return; }

    const int group                 = unit % (Geometry::HeadDim / kGqaKvNvfp4Group);
    const int tmp                   = unit / (Geometry::HeadDim / kGqaKvNvfp4Group);
