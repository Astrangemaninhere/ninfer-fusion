...
def fp8_row_scaled_stream(reader: MuseReader, key: str, n_src: int, n_total: int,
    ...
    for b0 in range(0, n_total, rows_per_block):
        ...
        # Defensive: e4m3 NaN word (0x7F) would poison the whole output row once
        # it reaches a multiply; replace with a zero word (drops that one element).
        if bool((codes == 0x7F).any()):
            codes = torch.where(codes == 0x7F, torch.zeros_like(codes), codes)
...
def encode_nvfp4_direct(reader: MuseReader, key: str, n: int, k: int,
    ...
    if divisor is None:
        # Engine NVFP4 math divides by the stored divisor (dequant * s1 / divisor);
        # the source scale_2 multiplies, so the stored word is its reciprocal.
        divisor = 1.0 / _scalar(reader, key, "weight_scale_2") if reader.contains(
...
def fused_gateup_payload(reader: MuseReader, layer: int, fmt: str) -> bytes:
    ...
    divisor = 1.0 / _scalar(reader, base + "gate_proj", "weight_scale_2") if reader.contains(
...
def convert(args) -> None:
    ...
    with ArtifactWriter(out, ArtifactIdentity(MODEL_ID, WEIGHTS_ID), plan) as writer:
        ...
        for index, spec in enumerate(plan, start=1):
            ...
            # DEBUG: nvfp4 divisor sanity (first bad object aborts loudly)
            if payload.__class__ is bytes and len(payload) > 8:
                import struct as _s
                _tail = _s.unpack('<f', payload[-4:])[0]
                if not (_tail > 1e-6 and _tail < 1e6):
                    print('BAD-DIVISOR', name, _tail, flush=True)
                    raise SystemExit(3)
            writer.write(name, payload)
            del payload
