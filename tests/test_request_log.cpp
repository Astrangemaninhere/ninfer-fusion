...
int main() {
    ...
    failures +=
        check(format_request_done(context, outcome).find(
                  "speculative=mtp spec_drafted=900 spec_accepted=720 spec_accept_rate=0.8000 "
                  "spec_rounds=300 spec_fallback_steps=2 spec_accept_len=3.40tok/round") !=
                  std::string::npos,
              "human request log omits the speculative acceptance counters");
    failures += check(done.at("speculative").at("accepted_tokens") == 720 &&
                          std::abs(done.at("speculative").at("acceptance_rate").get<double>() -
                                   0.8) < 1.0e-12,
                      "JSON request record omits the speculative acceptance rate");
