...
- 结论速览：**本仓没有 `NINFER_TARGET_SM_COUNT` 宏**（全仓 0 命中），SM 数全部以
  **字面常量**（170 / 85 / 42 / 340 / 680 / 510 / 680 / 1020）散布在 9 处代码点。
...
grep -rn -iE "occupanc|cudaGetDeviceProperties|cudaDeviceGetAttribute|props\." ...
grep -rn -iE "wave|persistent|resident_ctas|blocks_per_sm|ctas_per_sm" ...
```
...
| A2 | `src/ops/sparse_moe/prefill/sparse_moe_prefill_kernels.cu:259-261`（launch :1192,:1197,:1231,:1237,:1246,:1252） | `kRtx5090SmCount=170`、`kPrefillBlocksPerSm=3`、`kPrefillPersistentBlocks=510` | Q4/Q5/Q6 prefill GEMM 的持久网格（grid-stride loop），与 :264/:683 `__launch_bounds__(...,3)` 配套 | 是（纯性能；grid-stride 保证正确性） |
| A3 | `src/ops/gdn_gating_proj/bf16/bf16_gdn_gating_proj_plan.cpp:130`（340）、`:139`（340/680） | `340 = 170×2`、`680 = 170×4` | `cooperative_27/35_grid_is_resident` 判定 SplitK>1 的协作 kernel 网格是否驻留；对应 `bf16_gdn_gating_proj_kernels.cu:275-296`（`cudaLaunchAttributeCooperative` + `cudaLaunchKernelEx`）与 `bf16_gdn_gating_proj_gemm_mma.cuh:296`（`cooperative_groups::this_grid().sync()`） | **是，正确性相关**：非 5090 上若 plan 仍按 340/680 放行，协作启动会因网格超驻留直接失败（CUDA_CHECK abort） |
| A4 | `src/ops/kernel/gqa_attention_geometry.cuh:21` | `DecodeSplits = 85 × DecodeSplitScale`（85=170/2） | decode split-K 上限；host 侧 `gqa_attention_decode.cu:41,:59,:92`、`gqa_attention_decode_impl.cuh:47,:65,:98`；**device 侧** `gqa_attention_decode.cuh:81-93,:97-111`；同时经 `gqa_small_t_launch_capacity`（`gqa_attention_decode.cu:99`）决定 partial acc/m/l scratch 容量 | 是（SM 派生；device 侧使用 + 容量语义，改造要分两步） |
| A5 | `src/ops/softmax_attention/dense/causal_cache/geometry.cuh:12` | `SmallTMaximumSplits = 85 × SmallTSplitScale` | causal small-T split 上限；host `small_t.cu:23,:41,:48`；device `small_t.cuh:93,:127` | 同 A4 |
| A6 | `src/ops/kernel/bidirectional_gqa_attention.cuh:18` | `kBidirectionalGqaMaxSplit = 85` | split_capacity 校验上限（`launcher/bidirectional_gqa_attention.cu:104`）；当前 planner 的 split_limit≤64（`...cu:62-67`），85 实际达不到 | 是（SM 派生守卫，低影响） |
| A7 | `src/ops/softmax_attention/common/context_query.cuh:18` | `kContextQueryMaxSplit = 85` | 同上（`dense/context/launch.cu:104`） | 同 A6 |
...
| A9 | host：`gqa_attention_decode.cu:78`、`gqa_attention_decode_impl.cuh:84`、`causal_cache/small_t.cu:66`；device：`kernel/gqa_attention_decode.cuh:109`、`causal_cache/small_t.cuh:110` | `kMax = 42 × SplitScale`（42≈170/4，注释“keep the 8K grid at or below one 170-SM wave after accounting for the geometry's KV-head count”） | INT8 T=6、window 5000-8198 档的 split 数上限 | 是（SM 派生 cap，性能；device 侧有副本） |

明确**查不到**的：`1728`（只在 `eval/corpora/perplexity-1m/data/zhwiki/01.txt` 中文语料出现）、
`NINFER_TARGET_SM_COUNT`（本仓 0 命中）、fork 的 114/82/128/170 宏体系（上游无对应物；
`128` 在本仓全部是 tile/向量宽度，非 SM 数）。
...
### (b) 与 SM 数无关的固定常量（不要动）
- `src/ops/linear_add/w8/w8_linear_add_gemm_simt.cu:76`：`<<<2048/RowsPerCta,...>>>`，2048 是 kIntermediate（模型中间维），非 SM。
- `src/ops/sparse_moe/decode/sparse_moe_decode_kernels.cu:30-31`：`kAdaptiveD3Blocks=5×kIntermediate`、`kAdaptiveD4Blocks=5×(kHidden/4)`，工作量派生。
- `src/ops/sparse_moe/prefill/sparse_moe_prefill_kernels.cu:253-257`：kExpertBM/BN/BK=64、stages=2、warps=8 —— tile 常量。
...
- `src/ops/linear_pair/w8/w8_pair_plan.cpp:25-48`：路由表里的 673/680/784 等是 token 列区间边界（680 与 SM 无关，纯巧合）。
- `src/ops/common/bf16_vector.cuh:22-25`：`kBf16x8CacheSizedMaxElements=32Mi`，L2/缓存 regime 阈值。
- 单 CTA kernel：`launcher/scalar.cu:9-32`、`prefill_scan`（:1167）、`d2_warp`（decode:514）、`packed/launch.cu:43`。
...
- `src/ops/linear/nvfp4/nvfp4_config.h:195-210`：warp-count 交叉点，注释自述“RTX 5090 cold-cache winners / measured occupancy crossovers”。验证：nsight compute 对比寄存器/occupancy。
...
- 注意：`DeviceContext::sm()`（device.cu:122）返回 `major*10+minor`（compute capability，
  如 120），**不是** SM 数，不能误用。
- 引擎侧（src/）目前 **0 处**使用 `multiProcessorCount`。现成运行时范例在
  ...
  `bench/ops/gated_delta_net_bench.cu:889`、`src/serve/request_log.cpp:1037` 等）也各自
  `cudaGetDeviceProperties`，仅用于打印/记录。
- 障碍：ops 层 launch 链（`wrapper/sparse_moe.cpp:242`、`wrapper/rope.cpp:123,:140`、
  `gated_delta_net/chunked/launch.cu:72` 等）只穿透 `cudaStream_t`，不传 `DeviceContext&`。
  纯 host 规划器（`bf16_gdn_gating_proj_plan.cpp`、gqa/causal split planner）更没有设备句柄。
...
1. **加统一入口**：`src/core/device.{h,cu}` 增加
   `int device_sm_count();`（`cudaDeviceGetAttribute(&n, cudaDevAttrMultiProcessorCount, dev)`
...
3. **A1**：`output.cu:46` 的 `kTargetCtas` 改为局部
   ...
   与 `v.check_grid` 原样保留。
4. **A3（唯一正确性项，建议优先）**：`bf16_gdn_gating_proj_plan.cpp:130,:139` 的 340/680
   ...
   启动处（kernels.cu:275-296）已有 `CUDA_CHECK(cudaLaunchKernelEx(...))`，超驻留会以
