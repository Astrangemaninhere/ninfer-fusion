...
// A registered GQA geometry is the exact (q_heads, head_dim, kv_heads) triple. The
// q-head count alone no longer identifies one: 16 q-heads at head_dim 256 is both the
// 35B geometry (16/2, group 8) and Spark-X2.5 (16/4, group 4). The KV head count
// therefore always comes from the KV cache (or the append input) being addressed --
// never from q_heads -- and this function only proves that the triple is registered.
...
std::uint32_t validate_cache(const PagedKVLayerView& cache, std::int32_t kv_heads, const char* op) {
    ...
    const bool e8    = cache.dtype == DType::E8Kv;
    const bool packed16 = nvfp4 || fp8 || iso3;
    if ((cache.dtype != DType::BF16 && cache.dtype != DType::I8 && !packed16 && !e8) ||
        cache.num_kv_heads != kv_heads || cache.head_dim <= 0) {
    ...
    const DType code_dtype =
        cache.dtype == DType::I8
            ? DType::I8
            : (fp8 ? DType::FP8_E4M3FN
                   : (packed16 || e8 ? DType::U8 : DType::BF16));
    ...
    const std::int32_t code_leading =
        (nvfp4 || iso3 || e8) ? cache.head_dim / 2 : cache.head_dim;
    ...
    const std::int32_t groups = cache.head_dim / kQuantGroup;
...
void validate_attention_tensors(const Tensor& q, const Tensor& positions, const Tensor& out,
    ...
    // Scale is model semantics (1/sqrt(head_dim) optionally scaled by a
    // qk_scale_factor); the kernels consume the caller-supplied value.
    if (!std::isfinite(scale) || scale <= 0.0f) {
        throw std::invalid_argument(std::string(op) + ": scale must be finite and positive");
    }
    const std::int32_t q_heads  = q.ne[1];
    const std::int32_t kv_heads =
        registered_kv_heads(q_heads, cache.head_dim, cache.num_kv_heads, op);
    const std::int32_t tokens   = q.ne[2];
    if (tokens <= 0) { throw std::invalid_argument(std::string(op) + ": T must be positive"); }
    require_shape(q, cache.head_dim, q_heads, tokens, 1, op, "q");
    require_shape(positions, tokens, 1, 1, 1, op, "positions");
    require_shape(out, cache.head_dim, q_heads, tokens, 1, op, "out");
...
void validate_batched_attention_tensors(const Tensor& q, const Tensor& positions,
    ...
    require_shape(q, cache.head_dim, q_heads, width, batch, op, "q");
    ...
    require_shape(out, cache.head_dim, q_heads, width, batch, op, "out");
    require_contiguous_nonnull(q, op, "q");
    require_contiguous_nonnull(positions, op, "positions");
...
std::size_t gqa_attention_workspace_capacity_bytes(std::int32_t head_dim,
                                                   std::int32_t q_heads, DType cache_dtype,
                                                   GqaExecutionEnvelope envelope,
                                                   std::int32_t batch_size, std::int32_t min_width,
                                                   std::int32_t max_width) {
    if (head_dim <= 0) {
        throw std::invalid_argument("gqa_attention workspace: invalid head dim");
    }
    // No cache view reaches this query, so accept every registered geometry that can
    // legally be sized at this (head_dim, q_heads): 16 q-heads at head_dim 256 is both
    // 35B (16/2) and Spark-X2.5 (16/4). Sizing below uses the larger 35B split policy,
    // which is a legal upper bound for the 16/4 form because every split count is
    // non-decreasing in DecodeSplitScale (lower bound 4*S, key targets X/S, upper bound
    // 85*S), and the launched count is recomputed from the resolved geometry inside
    // gqa_attention_small_t_launch_for.
    if (!((q_heads == 24 && head_dim == 256) || (q_heads == 16 && head_dim == 256) ||
          (q_heads == 32 && head_dim == 128))) {
        throw std::invalid_argument(
            "gqa_attention workspace: unsupported Q/KV head geometry");
    }
    // E8Kv belongs in this set: the tier is a first-class KV storage (see the
    // geometry validation below and the E8 route forcing at the top of this
    // file). Leaving it out made a global --kv-dtype e8 abort warmup with
    // "invalid profile or interval" (_TODO.md 98/117 U2).
    ...
    const auto chunk_capacity = [&](std::int32_t width) {
        ...
        (void)allocate_small_t_workspace(layout, head_dim, q_heads, width, splits,
                                          batch_size);
...
void gqa_attention(const Tensor& q, const Tensor& k, const Tensor& v, const Tensor& positions,
    ...
    const std::int32_t batch    = q.ne[3];
    const std::int32_t kv_heads =
        registered_kv_heads(q.ne[1], cache.head_dim, cache.num_kv_heads, op);
    require_shape(k, cache.head_dim, kv_heads, width, batch, op, "k");
...
void gqa_kv_append(const Tensor& k, const Tensor& v, const Tensor& positions,
    ...
    require_shape(k, cache.head_dim, kv_heads, tokens, 1, op, "k");
    require_shape(v, cache.head_dim, kv_heads, tokens, 1, op, "v");
    require_shape(positions, tokens, 1, 1, 1, op, "positions");
    require_contiguous_nonnull(k, op, "k");
    require_contiguous_nonnull(v, op, "v");
    require_contiguous_nonnull(positions, op, "positions");
