#!/bin/bash
# 只做：增量编译 + 带 NINFER_DF2SEL 跑一轮 + 汇总（补丁已应用，不再动源码）。
set -u
R=/home/user/ninfer-fusion
J=/mnt/c/Users/User/Documents/ziqinzhang
LOG=$J/dl/scale_probe2.log
export PATH="/home/user/.local/bin:$PATH"
exec >> "$LOG" 2>&1
echo "=================================================================="
echo "=== scale probe run2 $(date '+%F %H:%M:%S') ==="
cd $R/build || exit 3
make ninfer -j2 2>&1 | tail -10
rc=${PIPESTATUS[0]}
echo "make rc=$rc"
if [ "$rc" -ne 0 ]; then echo BUILD_FAIL; exit 1; fi
...
grep df2sel $J/dl/scale_probe_stderr.log | head -14
echo
...
if not rows:
    ...
    print(f"steps with |pair| >= 0.25*uspan: {sum(1 for r in ratio if r >= 0.25)}/{len(rows)}")
    print(f"steps with |pair| >= uspan     : {sum(1 for r in ratio if r >= 1.0)}/{len(rows)}")
PY
echo SCALE_PROBE2_DONE
