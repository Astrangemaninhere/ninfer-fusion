...
void append_context_impl(DFlash2AppendContext& state, const Tensor& features,
    ...
    // Env-gated (NINFER_DF2FEAT): dump the feature chain for the first few calls, one
...
void propose_batch_impl(DFlash2BatchContext& state, qwen3_6::DFlashDecodeState& frame,
    ...
    // DFlash2 predicts the K masked columns (1..K) for width = K + 1; the bonus
    // token stays at column 0 and is not part of the proposal block.
    ...
    Tensor unary = state.execution.work.alloc(
        DType::FP32, {batch_size, steps, Config::selector_top_k});
    Tensor scores = state.execution.work.alloc(
        DType::FP32, {batch_size, steps, Config::selector_top_k, Config::selector_top_k});
