// qpn2_e2e_test.cu — prepack→qpn2 端到端: 机器实证映射 (qpn_map.cuh) 参考 vs 内核.
// M=8 满档 (qpn2 契约 M1..8); 固定 scale 与随机 scale 双跑.
...
int main() {
    ...
    for (int m = 0; m < M; ++m) {
        for (int n = 0; n < N; ++n) {
            float acc = 0.f;
            for (int d = 0; d < K; ++d) { acc += __half2float(xh[m * K + d]) * w[n * K + d]; }
    ...
    // host wrapper 目前只有 simt; 直接按内核 launch 语义调 qpn2 (SPLITK=1, NACC=1)
    {
        dim3 grid(N / 32), block(32);
        // 通过 host wrapper 不存在 -> 直接内联 launch (qpn2 模板在 kernels 头里)
        skinny_nvfp4_qpn2<1, 1><<<grid, block>>>(d_c, d_s, d_x, d_y, N, K, M, gscale);
    }
    ...
    std::printf("QPN2-E2E M=%d N=%d K=%d bad=%d %s\n", M, N, K, bad,
                bad == 0 ? "PASS" : "FAIL");
    return bad == 0 ? 0 : 1;
}
