...
// gelu is the exact erf activation (ACT2FN["gelu"]), evaluated from the represented inputs.
double gelu_mul_oracle_value(double gate, double up) {
    const double g = gate;
    return 0.5 * g * (1.0 + std::erf(g / std::sqrt(2.0))) * up;
}
...
std::vector<double> gelu_mul_oracle(const std::vector<float>& gate, const std::vector<float>& up) {
    ...
    for (std::size_t i = 0; i < gate.size(); ++i) {
        expected[i] = gelu_mul_oracle_value(gate[i], up[i]);
    }
    return expected;
}
...
int run_contiguous_case(const char* label, std::int32_t rows, std::int32_t columns,
    ...
    failures += verify_exact("gelu_mul gate unchanged",
                             from_device<std::uint16_t>(device_gate.data(), count), gate_bits);
    failures += verify_exact("gelu_mul up unchanged",
                             from_device<std::uint16_t>(device_up.data(), count), up_bits);
    failures += device_gate.verify_guards("gelu_mul gate");
    failures += device_up.verify_guards("gelu_mul up");
    failures += device_out.verify_guards("gelu_mul out");
    return failures;
}

// gate/up as the two halves of one packed [2*rows, columns] matrix (strided read-only inputs).
int run_strided_gate_up_case() {
    constexpr std::int32_t rows    = 10240;
    ...
    int failures =
        verify_pointwise("gelu_mul strided gate/up", from_device_bf16(device_out.data(), count),
                         expected, gelu_mul_bf16_criterion());
    failures += verify_exact("gelu_mul strided inputs unchanged",
                             from_device<std::uint16_t>(device_gate_up.data(), gate_up_bits.size()),
                             gate_up_bits);
    failures += device_gate_up.verify_guards("gelu_mul strided inputs");
    failures += device_out.verify_guards("gelu_mul strided out");
    return failures;
}

int run_edge_case() {
    std::vector<float> gate{-40.0f, -12.0f, -1.0f, 0.0f, 1.0f, 12.0f, 40.0f};
    std::vector<float> up{7.0f, -6.0f, 5.0f, -4.0f, 3.0f, -2.0f, 1.0f};
    round_to_bf16(gate);
    round_to_bf16(up);
    ...
    int failures = verify_pointwise("gelu_mul edge values",
                                    from_device_bf16(device_out.data(), expected.size()), expected,
                                    gelu_mul_bf16_criterion());
    failures += device_gate.verify_guards("gelu_mul edge gate");
    failures += device_up.verify_guards("gelu_mul edge up");
    failures += device_out.verify_guards("gelu_mul edge out");
    return failures;
}
...
int main() {
    ...
    int failures = 0;
    // Spark-X2.5 MLP activation: [10240, T] for decode and prefill widths.
    failures += run_contiguous_case("gelu_mul [10240,1]", 10240, 1, 101u);
    failures += run_contiguous_case("gelu_mul [10240,6]", 10240, 6, 102u);
    failures += run_contiguous_case("gelu_mul [10240,48]", 10240, 48, 103u);
