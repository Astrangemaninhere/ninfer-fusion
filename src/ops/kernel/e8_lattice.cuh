...
// Warp-cooperative 8D Hadamard rotation: each lane owns one coordinate of an
// 8-lane sub-block (sub_mask selects the block); returns the rotated value of
// the caller's own coordinate. Every lane redundantly runs the full butterfly
// for its block — fine for the KV write path.
__device__ __forceinline__ float hadamard_8d_warp_single(float x, int lane, unsigned sub_mask) {
    const int sub_lane = lane & 7;
    float vals[8];
    vals[sub_lane] = x;
#pragma unroll
    for (int off = 1; off < 8; off <<= 1) {
        vals[sub_lane ^ off] = __shfl_xor_sync(sub_mask, x, off);
    }
    float out[8];
    hadamard_rot_8d(vals, out);
    return out[sub_lane];
}
