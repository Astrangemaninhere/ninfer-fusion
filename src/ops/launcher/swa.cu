...
SwaPlan swa_resolve_plan(std::int32_t tokens, SwaContextExecutionEnvelope envelope,
    ...
    if (window != 2048 && window != 4096) {
        throw std::invalid_argument("swa plan: registered windows are 2048 and 4096");
    }
...
void swa_launch(const Tensor& q, const Tensor& query_k, const Tensor& query_v,
    ...
    dispatch_tokens(q.ne[2], [&]<int Tokens, int Warps>() {
        ...
        const auto launch_case = [&]<int Window>() {
            ...
            if (direct) {
                ...
                swa_split_partial_kernel<Window, Tokens, Warps, KeyBlock, true>
            ...
            swa_split_partial_kernel<Window, Tokens, Warps, KeyBlock, false>
            ...
            swa_reduce_kernel<Window, Tokens, KeyBlock, ReduceWarps>
        ...
        switch (plan.window) {
        case 2048:
            launch_case.template operator()<2048>();
            return;
        case 4096:
            launch_case.template operator()<4096>();
            return;
        default:
            throw std::invalid_argument("swa: unsupported window");
        }
    });
}
