...
Usage:
  ...
  python ple_reference.py replay <manifest> <eos> <T> <tok...> <prev...>
        # print rows for a real predecessor window; prevs is
        # (ngram-1)*T, token-major, oldest-first -- the layout
        # PleLayout::derive_rows consumes
"""
...
def main():
    ...
    if cmd == "gen":
        ...
        # The existing paths only ever run with prevs == {eos, eos}, so the
        # bigram/trigram mixers never see a real predecessor. This drives the
        # same reference formula with a real window.
        with open(sys.argv[2]) as fh:
            m = json.load(fh)
        eos = int(sys.argv[3])
        count = int(sys.argv[4])
        need = count * (NGRAM - 1)
        tokens = [int(v) for v in sys.argv[5:5 + count]]
        prevs = [int(v) for v in sys.argv[5 + count:5 + count + need]]
        if len(tokens) != count or len(prevs) != need:
            print("replay: expected %d tokens and %d prevs, got %d/%d"
                  % (count, need, len(tokens), len(prevs)), file=sys.stderr)
            return
        rows = derive_rows(tokens, prevs, eos, m["per_head_vocabulary_sizes"],
                           m["per_head_offsets"])
        for t, r in zip(tokens, rows):
            print(t, r)
    else:
