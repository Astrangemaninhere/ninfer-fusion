...
#pragma unroll
    for (int n = 0; n < PVNt; ++n) {
        ...
        if (prow0 < row_count) {
            int aq_head = 0;
            int atoken  = 0;
            causal_small_t_tc_row_to_qt<Geometry>(prow0, tokens, kv_head, aq_head, atoken);
            if (causal_valid_q_head<Geometry>(kv_head, aq_head)) {
                const std::int64_t dst =
                    causal_partial_acc_index<Geometry>(aq_head, d0, atoken, split, tokens);
        ...
        if (prow1 < row_count) {
            int aq_head = 0;
            int atoken  = 0;
            causal_small_t_tc_row_to_qt<Geometry>(prow1, tokens, kv_head, aq_head, atoken);
            if (causal_valid_q_head<Geometry>(kv_head, aq_head)) {
                const std::int64_t dst =
                    causal_partial_acc_index<Geometry>(aq_head, d0, atoken, split, tokens);
