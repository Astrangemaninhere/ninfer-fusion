...
// Speculative acceptance ratio shared by the human request-done line and the JSON record. It is
// the same ratio the DFlash2 demotion floor compares against, so the logged number and the
// runtime policy cannot drift apart. 0 when the request drafted nothing.
[[nodiscard]] double speculative_acceptance_rate(const GenerationMetrics& metrics) noexcept {
    if (metrics.speculative_draft_tokens == 0) { return 0.0; }
    return static_cast<double>(metrics.speculative_accepted_tokens) /
...
Json speculative_json(const GenerationMetrics& metrics) {
    return Json{{"backend", product::speculative_backend_name(metrics.speculative_backend)},
                ...
                {"acceptance_rate", metrics.speculative_draft_tokens == 0
                                        ? Json(nullptr)
                                        : Json(speculative_acceptance_rate(metrics))},
...
// Human-readable speculative accounting on the request-done line. Every enabled backend (mtp,
// dflash, dflash2) prints the same fields, so a throughput number is attributable without
// re-deriving it from the tok/round heuristic:
//   spec_drafted        draft tokens submitted to the verifier; the anchor is never drafted and a
//                       round that drafts nothing contributes 0
//   spec_accepted       draft tokens the verifier KEPT: the draft prefix that matched the target.
//                       The correction token committed at the first mismatch, or the bonus token
//                       after a full accept, is re-sampled by the target and is NOT counted, so a
//                       round commits spec_accepted + 1 tokens and a partially accepted round
//                       contributes exactly its accepted prefix
//   spec_accept_rate    spec_accepted / spec_drafted, in [0,1]; 0 when nothing was drafted
//   spec_rounds         verifier rounds that drafted at least one token
//   spec_fallback_steps rounds that drafted nothing (DFlash2 acceptance floor, context demotion)
//   spec_accept_len     mean committed tokens per drafting round, 1 + accepted / rounds
std::string speculative_str(const GenerationMetrics& metrics) {
    ...
    out << product::speculative_backend_name(metrics.speculative_backend)
        << " spec_drafted=" << metrics.speculative_draft_tokens
        << " spec_accepted=" << metrics.speculative_accepted_tokens << " spec_accept_rate="
        << std::fixed << std::setprecision(4) << speculative_acceptance_rate(metrics)
        << " spec_rounds=" << metrics.speculative_rounds
        << " spec_fallback_steps=" << metrics.speculative_fallback_steps;
    if (metrics.speculative_rounds > 0) {
        out << " spec_accept_len=" << std::setprecision(2)
